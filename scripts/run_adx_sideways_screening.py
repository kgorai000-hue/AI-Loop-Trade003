"""Screen adx_sideways_threshold with per-symbol best adx_trend fixed."""
...
from scripts.screening_params import BEST_ADX_TREND
...
SIDEWAYS_VALUES = (15, 18, 20, 22, 24)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="ADX sideways threshold grid")
    ...
    out_path = (
        PROJECT_ROOT
        / f"data/reports/screening_{timeframe}_{args.bars}bars_nogate_adxsideways_{stamp}.xlsx"
    )
    out_path.parent.mkdir(parents=True, exist_ok=True)

    print("=== ADX sideways_threshold grid ===")
    print(f"  symbols    : {len(symbols)}")
    print(f"  timeframe  : {timeframe}")
    print(f"  bars       : {args.bars}")
    print(f"  MC sims    : {args.mc_sims}")
    print("  strategy   : trend_following only")
    print("  ADX trend  : per-symbol best map (fixed)")
    print(f"  sideways   : {', '.join(str(v) for v in SIDEWAYS_VALUES)}")
    ...
    for sideways in SIDEWAYS_VALUES:
        config.strategies.adx_sideways_threshold = float(sideways)
        print(f"\n=== sideways={sideways} ===", flush=True)

        for idx, symbol in enumerate(symbols, start=1):
            trend_adx = BEST_ADX_TREND.get(symbol, config.strategies.adx_trend_threshold)
            config.strategies.adx_trend_threshold = float(trend_adx)
            agent = BacktestAgent(config, store)
            print(
                f"  [{idx}/{len(symbols)}] {symbol} (trend_ADX={trend_adx:g}) ...",
                flush=True,
            )
            try:
                ...
                msg = f"sideways={sideways} {symbol}: {type(exc).__name__}: {exc}"
                errors.append(msg)
    ...
    pivot = detail.pivot_table(
        index="symbol",
        columns="adx_sideways_threshold",
        ...
        [
            {
                ...
                "strategy": "trend_following",
                "sideways_values": ",".join(str(v) for v in SIDEWAYS_VALUES),
                "error_count": len(errors),
                "note": (
                    "Gates OFF; ADX trend fixed per-symbol best; "
                    "sideways wired as flatten when ADX < threshold"
                ),
            }
        ]
    )
    ...
    print("\n=== Best sideways per symbol (by return) ===")
    print(
        best[
            [
                "symbol",
