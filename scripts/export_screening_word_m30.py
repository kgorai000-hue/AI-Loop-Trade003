...
XLSX = PROJECT_ROOT / "data/reports/screening_M30_2000bars_20260715_125027.xlsx"
...
def main() -> None:
    ...
    doc = Document()
    doc.add_heading("MT5_loop 全銘柄スクリーニング報告書（M30）", 0)
    doc.add_paragraph("条件: M30 / 2000本 / Monte Carlo 100回 / 12銘柄順次実行")
    doc.add_paragraph("生成日時: 2026-07-15（H1・M15と同条件・時間足のみ変更）")
    ...
    doc.add_heading("4. 時間足比較", level=1)
    doc.add_paragraph("- H1 (2000本): ゲート合格 0/12")
    doc.add_paragraph("- M15 (2000本): ゲート合格 2/12（#USSPX500, #UK100）")
    doc.add_paragraph(f"- M30 (2000本): ゲート合格 {n_pass}/12（#USSPX500, #Japan225, #UK100）")
    doc.add_paragraph("- 合格戦略はいずれも mean_reversion")
